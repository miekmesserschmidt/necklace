...
print(rad_symbols)
r0, r1, r2, r3, r4, *rest = rad_symbols
...
def trig_order(degree_list: list[int]):
    sins = degree_list[: len(sin_symbols)]
    coss = degree_list[len(sin_symbols) :]

    sins_tup = tuple(-i for i in reversed(sins))
    coss_tup = tuple(-i for i in reversed(coss))

    return (sum(sins), sins_tup, sum(coss), coss_tup)


# %%
trig_system
# %%
Gtrig = sympy.groebner(trig_system, *(sin_symbols + cos_symbols), order=trig_order)
for p in Gtrig:
    print(p)
