from necklace.compute import corona, triangle
...
def test_all_triangles2():
    t = list(triangle.all_triangles([0, 1]))
    t.sort()
    assert t == [
        Triangle(0, 0, 0),
        Triangle(1, 0, 0),
...
def test_all_triangles3():
    t = list(triangle.all_triangles([0, 1, 2]))
    t.sort()
    assert t == [
        Triangle(0, 0, 0),
        #
...
def test_matrix():
    ...
    M = triangle.matrix(c, mms)
    print(M)

    assert M == sympy.Matrix([[0, 0, 1, 0, 2, 0]])
